...
public class ChangePasswordServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            RegisterUserBean ob = new RegisterUserBean();
            String npass = request.getParameter("NewPassword");
            String retypenpass = request.getParameter("ConfirmPassword");
            PrintWriter out = response.getWriter();
            if(!npass.equals(retypenpass)){
                     response.sendRedirect("WebContent/ChangePassword.jsp");
                }
            else{
            HttpSession session = request.getSession();
                ...
                ob.setRegisterUserID(currentuser.getRegisterUserID());
                ob.setPassword(npass);
                int n = RegisterUserDAO.changePassword(ob);
                if(n>0)
